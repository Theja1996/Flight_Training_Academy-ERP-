...
function CreateInstructor() {
  ...
  function docreateInstructor(values) {
    setLoading(true);
    getMaxId("FlightInstructor")
      .then((id) => {
        console.log(id);
        let v = { ...values };
        v.key = id;
        v.instructortImage = "1";
        add(v, "FlightInstructor")
          .then(() => {
            setLoading(false);
                ...
                <Form.Item shouldUpdate={true}>
                  {() => (
                    <Button loading={loading} type="primary" htmlType="submit">
                      <CheckOutlined /> Create Instructor
                    </Button>
                  )}
                </Form.Item>
